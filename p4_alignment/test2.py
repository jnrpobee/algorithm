def align(
    ...
    dp = {(i, j): float('inf') for i in range(m + 1) for j in range(n + 1)}
    ...
    if banded_width == -1:
        ...
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                match = dp.get((i - 1, j - 1), float('inf')) + (match_award if seq1[i - 1] == seq2[j - 1] else sub_penalty)
    ...
    while i > 0 or j > 0:
        ...
        if i > 0 and j > 0 and (seq1[i - 1] == seq2[j - 1] or current_cost == dp.get((i - 1, j - 1), float('inf')) + sub_penalty):
            alignment_seq1.append(seq1[i - 1])
            alignment_seq2.append(seq2[j - 1])
            i -= 1
            j -= 1
        elif j > 0 and current_cost == dp.get((i, j - 1), float('inf')) + indel_penalty:
