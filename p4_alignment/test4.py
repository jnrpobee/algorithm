def align(
    ...
    if banded_width == -1:
        ...
        dp = [[float('inf')] * (2 * banded_width + 1) for _ in range(m + 1)]
    ...
    if banded_width == -1:
        ...
        for i in range(1, m + 1):
            for j in range(max(1, i - banded_width), min(n + 1, i + banded_width + 1)):
                band_j = j - (i - banded_width)
                if 0 <= band_j < len(dp[0]):
                    if 0 <= band_j - 1 < len(dp[0]):
                        match = dp[i - 1][band_j - 1] + (match_award if seq1[i - 1] == seq2[j - 1] else sub_penalty)
                    else:
                        match = float('inf')
                    delete = dp[i - 1][band_j] + indel_penalty
                    insert = dp[i][band_j - 1] + indel_penalty if band_j - 1 >= 0 else float('inf')
                    dp[i][band_j] = min(match, insert, delete)
    ...
    while i > 0 or j > 0:
        if banded_width == -1:
            ...
            band_j = j - (i - banded_width)
            current_cost = dp[i][band_j] if 0 <= band_j < len(dp[0]) else float('inf')

        print(f"i: {i}, j: {j}, current_cost: {current_cost}")

        if i > 0 and j > 0 and (seq1[i - 1] == seq2[j - 1] or (i > 0 and j > 0 and (i-1 >= 0 and j-1 >= 0 and current_cost == dp[i - 1][j - 1] + sub_penalty))):
            alignment_seq1.append(seq1[i - 1])
            alignment_seq2.append(seq2[j - 1])
            i -= 1
            j -= 1
        elif j > 0 and current_cost == dp[i][j - 1] + indel_penalty:
    ...
    alignment_cost = dp[m][n] if banded_width == -1 else dp[m][banded_width] if banded_width < n else dp[m][n]

    # Return the alignment cost and sequences
