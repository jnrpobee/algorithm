...
def branch_and_bound(edges: list[list[float]], timer: Timer) -> list[SolutionStats]:
    ...
    print("Greedy stats:", greedy_stats)
    ...
    def reduce_matrix(matrix):
        ...
        for r in range(n):
            for c in range(n):
                if row_min[r] != float('inf'):
                    matrix[r][c] -= row_min[r]

        col_min = [min(matrix[r][c] for r in range(n)) for c in range(n)]
        for c in range(n):
            for r in range(n):
                if col_min[c] != float('inf'):
                    matrix[r][c] -= col_min[c]

        return sum(row_min) + sum(col_min)
    
    stack = [([0], reduce_matrix([row[:] for row in edges]), [row[:] for row in edges])]

    while stack: 
        if timer.time_out():
            break

        tour, cost, matrix = stack.pop()
        ...
        for next_city in range(n):
            if next_city not in tour:
                ...
                if new_cost < best_score:
                    n_nodes_expanded += 1
                    stack.append((tour + [next_city], new_cost, new_matrix))
                else:
                    n_nodes_pruned += 1
                    cut_tree.cut(tour + [next_city])
